...
7. **Source of truth** — Respect the governance model: `.github/` for policy, `data/content/authored/` for authored work, `data/roles/` for operational prompts, `data/domains/` for schemas.
...
When MCP servers have been published by `src/automation/sync-agents.ps1`, treat them as user-scoped capabilities that are available from any repository on the same machine. Still resolve files, governance, manifests, and private/public eligibility from the active repository before implementing or wiring connector behavior.
...
- [`data/config/mcp/`](../../../config/mcp/) — MCP configuration
- [`data/config/mcp/README.md`](../../../config/mcp/README.md) — MCP architecture guide
- [`src/automation/mcp-servers/`](src/automation/mcp-servers/) — MCP server implementations
