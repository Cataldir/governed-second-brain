...
**Rule**: Always provide a structured brief when delegating — include scope, acceptance criteria, and constraints. Consult `agents/team-mapping.md` for the full agent registry.
...
1. **Resolve canonical names first** — Load `agents/team-mapping.md` and use the `Agent Name` values (for example, `SystemArchitect`, `TypeScriptDeveloper`, `PlatformEngineer`).
